...
export const DGIIReportView: React.FC = () => {
  ...
  const fetchReport = async () => {
    setIsLoading(true);
    try {
      const res = await fetch('/api/reports/dgii-606');
      if (res.ok) {
  ...
  const handleDownloadTXT = () => {
    ...
    const headerLine = `606|${organization?.rnc.replace(/-/g, '')}|${period.replace('-', '')}|${reportData.records.length}`;
    const recordLines = reportData.records.map((r: any) => 
      `${r.rnc_cedula}|${r.tipo_id}|${r.tipo_bienes_servicios}|${r.ncf}|${r.ncf_modificado}|${r.fecha_comprobante}|${r.fecha_pago}|${r.monto_servicios.toFixed(2)}|${r.monto_bienes.toFixed(2)}|${r.total_monto_facturado.toFixed(2)}|${r.itbis_facturado.toFixed(2)}|0.00|0.00|0.00|${r.itbis_facturado.toFixed(2)}|0.00|0.00||${r.forma_pago}`
    );
    ...
    a.href = url;
    a.download = `606_${organization?.rnc.replace(/-/g, '')}_${period.replace('-', '')}.txt`;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold text-xs shadow-sm"
          >
            <Download className="w-4 h-4" />
            <span>Descargar Archivo 606 (.TXT)</span>
          </button>
        </div>
      </div>

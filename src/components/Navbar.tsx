        ...
        <div className="relative" ref={userMenuRef}>
          ...
          {isUserMenuOpen && (
            <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 p-2 z-30 animate-in fade-in zoom-in-95 duration-100 text-xs">
              ...
              {/* Portal Switcher — Super Admin ONLY visible to platform-level users */}
              <div className="p-1 space-y-1 mt-1 border-b border-slate-100 dark:border-slate-800">
                ...
                {/* Only render Super Admin option if user has platform-level role */}
                {(currentUser?.platform_role === 'SUPER_ADMIN' || currentUser?.platform_role === 'PLATFORM_ADMIN') && (
                  <button
                    onClick={() => {
              ...
              {/* Role info display only — no simulation switcher in production */}
              <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-800">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">
                  Rol Actual
                </span>
                <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">
                  {currentUser?.role === 'ADMIN' ? 'Administrador General'
                    : currentUser?.role === 'ACCOUNTANT' ? 'Contabilidad & DGII'
                    : currentUser?.role === 'SUPERVISOR' ? 'Supervisor'
                    : 'Empleado Operativo'}
                </span>
                {currentUser?.platform_role && currentUser.platform_role !== 'NONE' && (
                  <span className="ml-2 text-[10px] font-bold px-1.5 py-0.5 bg-indigo-900/60 text-indigo-300 border border-indigo-700/60 rounded">
